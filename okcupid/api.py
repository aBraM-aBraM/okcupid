...
class OkCupidClient:
    def __init__(self):
        ...
        self._logger = logger.setup_logger(__name__)
    ...
    def send_msg(self, user: User, msg: str):
        body = f"""
    {{"operationName":"SendMessage","variables":{{"input":{{"text":{msg},"targetId": {user.user_id},"gif":null,}}}},"query":"mutation SendMessage($input: ConversationMessageSendInput!) {{ conversationMessageSend(input: $input) {{ success nway messageId threadId adTrigger ratingTrigger }} }}"}}
"""
        self._logger.info(f"sending message \"{msg}\" to {user.name} ({user.user_id})")
